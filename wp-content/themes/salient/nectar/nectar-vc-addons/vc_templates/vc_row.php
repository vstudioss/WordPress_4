<?php
   ...
   extract(shortcode_atts(array(
    ...
    echo'
	<div id="'.$row_id.'" data-midnight="'.strtolower($midnight_color).'" data-bg-mobile-hidden="'.$background_image_mobile_hidden.'" class="wpb_row vc_row-fluid vc_row '. $main_class . $equal_height_class . $parallax_class . ' ' . $vertically_center_class . ' '. $class . ' " '.$using_custom_text_color.' style="'.$style.'">';
	
	//row bg 
	echo '<div class="row-bg-wrap"> <div class="row-bg '.$using_image_class . ' ' . $using_bg_color_class . ' '. $etxra_class.'" '.$parallax_speed.' style="'.$bg_props.'" data-color_overlay="'.$color_overlay.'" data-color_overlay_2="'.$color_overlay_2.'" data-gradient_direction="'.$gradient_direction.'" data-overlay_strength="'.$overlay_strength.'" data-enable_gradient="'.$enable_gradient.'"></div> </div>';
